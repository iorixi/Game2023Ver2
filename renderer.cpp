...
ID3D11RasterizerState* Renderer::m_rssolid{};
ID3D11RasterizerState* Renderer::m_rswire{};
...
void Renderer::Init(Application* ap)
{
...

	{
		...
		m_Device->CreateRasterizerState(&rasterizerDesc, &m_rssolid);
	}

	{
		// ラスタライザステート設定(WIREFRAME)
		D3D11_RASTERIZER_DESC rasterizerDesc{};
		rasterizerDesc.FillMode = D3D11_FILL_WIREFRAME;
		rasterizerDesc.CullMode = D3D11_CULL_BACK;
		rasterizerDesc.DepthClipEnable = TRUE;
		rasterizerDesc.MultisampleEnable = FALSE;
		m_Device->CreateRasterizerState(&rasterizerDesc, &m_rswire);
	}

	m_DeviceContext->RSSetState(m_rssolid);

...
const float FurSupecularPower = 10;      // 毛の光る範囲
const float FurFlowScale = float2(500,1); // 毛の流れる量
const float3 FurColor = float3(1.0, 1.0, 1.0); // 毛の色

int FurShellCount = 50; // FurShellの枚数
const float FurLength = 0.1/25.0;  // FurShellの幅
...
texture2D fur_tex <
   string ResourceName = "a_tex_fur_neko.tga";// Fur発生テクスチャ。付属のテクスチャは服のボタン部分は黒くなっている
   int Miplevels = 1;
>;

...
ワールド変換行列を作る
--------------------------*/
void DX11MakeWorldMatrix(
	DirectX::SimpleMath::Matrix& inoutmat, 
...
ワールド変換行列を作る
--------------------------*/
void DX11MakeWorldMatrix(XMFLOAT4X4& inoutmat, XMFLOAT3& scale,XMFLOAT3& angle, XMFLOAT3& trans)
{
	ALIGN16 XMMATRIX mtxScale{};
	ALIGN16 XMMATRIX mtxRotX{};
	ALIGN16 XMMATRIX mtxRotY{};
	ALIGN16 XMMATRIX mtxRotZ{};
	ALIGN16 XMMATRIX mtxTrans{};

	ALIGN16 XMMATRIX wmtx{};

	mtxScale = XMMatrixScaling(scale.x, scale.y, scale.z);

	mtxRotX = XMMatrixRotationX((angle.x * XM_PI) / 180.0f);
	mtxRotY = XMMatrixRotationY((angle.y * XM_PI) / 180.0f);
	mtxRotZ = XMMatrixRotationZ((angle.z * XM_PI) / 180.0f);
	mtxTrans = XMMatrixTranslation(trans.x, trans.y, trans.z);

	wmtx = mtxScale * mtxRotX * mtxRotY * mtxRotZ * mtxTrans;

	XMStoreFloat4x4(&inoutmat, wmtx);
}
	...
	XMFLOAT3& trans)
{
	...
	mtxRotX = XMMatrixRotationX((angle.x));
	mtxRotY = XMMatrixRotationY((angle.y));
	mtxRotZ = XMMatrixRotationZ((angle.z));
	mtxTrans = XMMatrixTranslation(trans.x, trans.y, trans.z);
